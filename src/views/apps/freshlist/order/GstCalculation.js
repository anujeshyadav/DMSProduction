...
if (SuperGst == partyGst) {
  ...
  gstDetails = product?.map((ele, i) => {
    ...
    let costPrice = Number(
      (
        ele?.price /
        (DiscountPercentage * ((100 + Number(ele?.productData?.GSTRate)) / 100))
      ).toFixed(2)
    );
    let Cp_without_Discount = Number(
      (ele?.price / ((100 + Number(ele?.productData?.GSTRate)) / 100)).toFixed(
        2
      )
    );
  ...
  gstDetails = product?.map((ele, i) => {
    ...
    let total = Number((costPrice * ele?.qty * (100 + wholeGst)) / 100).toFixed(
      2
    );
    ...
    return {
      ...
      igstTax: [
        {
          rate: wholeGst,
          amount: Number(((costPrice * ele?.qty * wholeGst) / 100).toFixed(2)),
        },
      ],
    };
  });
