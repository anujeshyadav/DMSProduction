...
let gstDetails = [];
export const PurchaseGstCalculation = (Party, product, Context, Charges) => {
  let IgstTotal = 0;
  let CgstTotal = 0;
  let SgstTotal = 0;
  let Amount = 0;
  ...
  // debugger;
  let Tax = {
    maxGst: maxGst?.gstPercentage,
    OtherGSTCharges: OtherGSTCharges,
    TotalOtherCharges: Number((Charges + OtherGSTCharges).toFixed(2)),
    Charges: Charges,
    IgstTaxType: IgstTaxType,
    RoundOff: Number(RoundOff.toFixed(2)),
    GrandTotal: Number(GrandTotal?.toFixed(2)),
    Amount: Number(Amount?.toFixed(2)),
    IgstTotal: IgstTaxType
      ? Number((IgstTotal + OtherGSTCharges)?.toFixed(2))
      : Number(IgstTotal.toFixed(2)),
    CgstTotal: IgstTaxType
      ? Number(CgstTotal.toFixed(2))
      : Number((CgstTotal + OtherGSTCharges)?.toFixed(2)),
    SgstTotal: IgstTaxType
      ? Number(SgstTotal.toFixed(2))
      : Number((SgstTotal + OtherGSTCharges)?.toFixed(2)),
  };
