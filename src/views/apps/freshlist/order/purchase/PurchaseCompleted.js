...
class PurchaseCompleted extends React.Component {
  ...
  async Apicalling(id, db) {
    ...
    await PurchaseOrderList(id, db)
      .then((res) => {
        ...
        if (pendingStatus) {
          this.setState({ rowData: pendingStatus?.reverse() });
        }
  ...
  exportToPDF = async () => {
    ...
    await exportDataToPDF(csvData, "PurchaseCompletedList");
  };
  processCell = (params) => {
    return params.value;
  };
  ...
  exportToExcel = async (e) => {
    ...
    await exportDataToExcel(CsvData, "PurchaseCompletedList");
  };
  ...
  convertCSVtoExcel = async () => {
    ...
    await convertDataCSVtoExcel(CsvData, "PurchaseCompletedList");
  };
  ...
  convertCsvToXml = async () => {
    ...
    await convertDataCsvToXml(CsvData, "PurchaseCompletedList");
  };
