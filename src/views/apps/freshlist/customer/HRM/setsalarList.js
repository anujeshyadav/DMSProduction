...
class Setsalarlist extends React.Component {
  ...
  async Apicalling(id, db) {
    ...
    await _Get(URl, db, id)
      .then((res) => {
        ...
        if (res?.ApplyRule) {
          this.setState({ rowData: res?.ApplyRule });
          this.setState({ rowAllData: res?.ApplyRule });
        }
        this.setState({ AllcolumnDefs: this.state.columnDefs });
        this.setState({ SelectedCols: this.state.columnDefs });

        let userHeading = JSON.parse(localStorage.getItem("SetSalaryList"));
  ...
  async componentDidMount() {
    const InsidePermissions = CheckPermission("Set Salary");
